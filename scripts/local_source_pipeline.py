...
def split_text(text: str, max_tokens: int = HARD_LIMIT_TOKENS) -> list[str]:
    ...
    for chunk in chunks:
        if estimate_tokens(chunk) > max_tokens:
            safe_chunks.extend(split_by_window(chunk, max_tokens))
        else:
...
def split_by_window(text: str, max_tokens: int) -> list[str]:
    # Chinese token estimates are roughly one token per CJK character in this
    # pipeline, so the fallback character window must be smaller than max_tokens.
    chars_per_token = 0.75 if detect_language(text) == "zh" else 3.2
...
def collect_segments(slug: str) -> tuple[list[Segment], list[dict[str, Any]]]:
    ...
    text_companion_stems = {
        p.stem for p in files if p.suffix.lower() in SUPPORTED_TEXT_EXTS
    }
    ...
    for path in files:
        ...
        if ext in SUPPORTED_PDF_EXTS and path.stem in text_companion_stems:
            file_meta["status"] = "skipped_text_companion"
            file_meta["chunk_count"] = 0
            file_meta["text_companion"] = f"{path.stem}.txt"
...
def sanitize_extract(ext: dict[str, Any]) -> dict[str, Any]:
    content_type = ext.get("content_type") or ext.get("type") or "analysis"
